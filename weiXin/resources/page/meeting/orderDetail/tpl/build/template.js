...
/**/) {
'use strict';var $utils=this,$helpers=$utils.$helpers,errMsg=$data.errMsg,$string=$utils.$string,getImgPath=$helpers.getImgPath,status=$data.status,is=$data.is,increment_services=$data.increment_services,JSON=$data.JSON,e=$data.e,increment_service_names=$data.increment_service_names,i=$data.i,desc_time_status=$data.desc_time_status,desc_status=$data.desc_status,isBottomVisible=$data.isBottomVisible,time_status=$data.time_status,desc_text=$data.desc_text,parseInt=$data.parseInt,expire_minutes=$data.expire_minutes,reserve_date=$data.reserve_date,start_time=$data.start_time,end_time=$data.end_time,reserve_start_time=$data.reserve_start_time,g=$data.g,reserve_end_time=$data.reserve_end_time,meeting_room_list_img_url=$data.meeting_room_list_img_url,meeting_room_name=$data.meeting_room_name,meeting_room_address=$data.meeting_room_address,meeting_room_person_limit=$data.meeting_room_person_limit,refund_reason=$data.refund_reason,refuse_reason=$data.refuse_reason,basic_price=$data.basic_price,j=$data.j,v=$data.v,total_price=$data.total_price,bake=$data.bake,theme=$data.theme,persons=$data.persons,contact_name=$data.contact_name,contact_mobile=$data.contact_mobile,contact_email=$data.contact_email,is_need_invoice=$data.is_need_invoice,invoice_type=$data.invoice_type,invoice_title=$data.invoice_title,invoice_mail_address=$data.invoice_mail_address,invoice_tax_no=$data.invoice_tax_no,id=$data.id,$out='';if(errMsg){
...
var desc_time_status= ['','未开始','进行中','已结束'];
var desc_status ={0:'待支付',1:'已支付',2:'退款申请中',3:'退款审核通过',4:'退款审核拒绝',9:'已超时取消'};
var isBottomVisible = (0==status)||(1==status&&1==time_status)||9==status;
var desc_text = {
...
$out+='</span>\n            </div>\n        </div>\n\n\n        <div class="c_room_paytop">\n            <div class="c_room_paytop_inner cc">\n                <div class="c_room_paytop_inner_left"><a href="javascript:void(0);"><img src="';
...
$out+=$string(v.name);
$out+='</p>\n                    </div>\n                    <div class="weui-cell__ft">&yen;';
$out+=$string(v.price||0);
$out+='</div>\n                </div>\n                ';
}
$out+='\n                ';
}
$out+='\n                <div class="weui-cell">\n                    <div class="weui-cell__bd">\n                        <p></p>\n                    </div>\n                    <div class="weui-cell__ft">合计：<label style="font-size: 16px;color: #ff0234;">&yen;';
...
$out+='</div>\n    <div class="footer_fixed_box_right"><a href="javascript:;" style="cursor:pointer;" data-id="';
$out+=$string(id);
$out+='" class="j-pay footer_fixed_box_right_inner room_pay">确认支付</a></div>\n</div>\n';
}else if(1==status&&1==time_status){
$out+='\n<div class="footer_fixed_box">\n    <div class="footer_fixed_box_right" style="float: right;"><a href="javascript:;" style="cursor:pointer;" class="j-refund footer_fixed_box_right_inner room_pay">退款</a></div>\n</div>\n';
}else if(9==status){
$out+='\n<div class="footer_fixed_box">\n    <div class="footer_fixed_box_right" style="float: right;"><a href="javascript:;" style="cursor:pointer;" class="j-delete footer_fixed_box_right_inner room_pay">删除订单</a></div>\n</div>\n';
}
$out+='\n\n    <div class="c_activity_getscorepopup none j-reson">\n        <div class="c_activity_getscorepopup_inner">\n            <div class="c_activity_getscorepopup_inner_title">退款原因</div>\n            <textarea class="c_activity_getscorepopup_inner_remind j-c"  type="text" maxlength="500" placeholder="请填写退款原因" ></textarea>\n            <div class="c_activity_getscorepopup_inner_btns">\n                <a href="javascript:;" class="c_activity_getscorepopup_inner_btns_close j-close">取消</a>\n                <a href="javascript:;" class="c_activity_signcon_inner_btns_tosee j-con">确定</a>\n            </div>\n        </div>\n    </div>\n\n\n</div>\n\n';
}
