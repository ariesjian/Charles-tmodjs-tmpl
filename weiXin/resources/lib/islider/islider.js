...
iSlider.prototype._animateFuncs = {
    ...
    'depth': function (dom, axis, scale, i, offset) {
        var rotateDirect = (axis == "X") ? "Y" : "X";
        var zoomScale = (4 - Math.abs(i - 1)) * 0.22;
        ...
        dom.style.webkitTransform = 'scale('+ zoomScale + ', '+ zoomScale + ') translateZ(0) translate' + axis + '(' + (offset + 1.085 * scale * (i - 1)) + 'px)';
    },
...
iSlider.prototype._renderItem = function (i) {
    ...
    if (this.type === 'pic') {
        ...
        ? '<img class="j-link" style="cursor: pointer;" height="' + this.height + '" src="' + item.content + '">'
        : '<img class="j-link" style="cursor: pointer;" width="' + this.width + '" src="' + item.content + '">';
